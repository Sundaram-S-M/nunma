...
const DocumentModuleUploader: React.FC<DocumentModuleUploaderProps> = ({ courseId, chapterId, onClose, onSuccess }) => {
  ...
  const processUpload = async () => {
    ...
    uploadTask.on('state_changed',
      ...
      (uploadError) => { console.error(uploadError); setError('Upload failed.'); setIsUploading(false); },
      async () => {
        try {
          ...
          onSuccess();
          onClose();
        } catch (err: any) { setError('Failed to save record.'); setIsUploading(false); }
      }
    );
  };
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-[#040457]/80 backdrop-blur-xl animate-in fade-in duration-300">
      <div className="bg-white rounded-[3rem] w-full max-w-lg shadow-2xl overflow-hidden p-10 relative">
        ...
        <div className="flex gap-4 mt-8">
          <button onClick={onClose} disabled={isUploading} className="flex-1 py-5 bg-gray-50 text-gray-400 rounded-[2rem] font-black uppercase text-xs tracking-widest hover:bg-gray-100 transition-all disabled:opacity-50">Cancel</button>
