...
exports.gradePdfSubmission = (0, https_1.onCall)({
    ...
    // Step 1: Auth Check
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Thala or student authentication required.");
    }
    const { zoneId, examId, learnerId, filePath } = request.data;
    if (!zoneId || !examId || !learnerId || !filePath) {
        throw new https_1.HttpsError("invalid-argument", "Missing required parameters (zoneId, examId, learnerId, filePath).");
    }
    ...
    let base64Pdf;
    try {
        const response = await axios_1.default.get(bunnyPdfUrl, {
        ...
        throw new https_1.HttpsError("internal", "Could not retrieve the answer script from storage.");
    }
    // Step 3: Fetch Exam Rubric from Firestore
    const examSnap = await db.doc(`zones/${zoneId}/exams/${examId}`).get();
    if (!examSnap.exists) {
        throw new https_1.HttpsError("not-found", "The specified exam does not exist.");
    }
    const examData = examSnap.data();
    const rubric = examData === null || examData === void 0 ? void 0 : examData.rubric; // Fetched from the exams doc
    if (!rubric) {
        throw new https_1.HttpsError("failed-precondition", "No grading rubric found for this exam.");
    }
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", "The AI evaluator encountered an error during script analysis.");
    }
});
