...
const LaunchZone: React.FC = () => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => setZoneImage(reader.result as string);
      reader.readAsDataURL(file);
    }
  };

  const handleCreateZone = () => {
    if (!zoneTitle || !zonePrice) return;
    setIsLaunching(true);
    const newZone = {
      ...
      students: 0,
      image: zoneImage || `https://picsum.photos/seed/${Date.now()}/400/300`
    };
    
    const savedZones = localStorage.getItem(ZONES_STORAGE_KEY);
    const zonesList = savedZones ? JSON.parse(savedZones) : [];
    const updated = [...zonesList, newZone];
      ...
      <div className="bg-white rounded-[4rem] border border-gray-100 shadow-[0_40px_120px_rgba(0,0,0,0.06)] overflow-hidden">
        ...
        <div className="p-14 space-y-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <div className="space-y-10">
              <div>
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 block ml-1">Zone Visual</label>
                <div 
                  onClick={() => imageInputRef.current?.click()} 
                  className="aspect-video bg-gray-50 border-2 border-dashed border-gray-200 rounded-[3rem] flex flex-col items-center justify-center cursor-pointer hover:border-[#c1e60d] hover:bg-white transition-all relative overflow-hidden group shadow-inner"
                >
                  {zoneImage ? (
